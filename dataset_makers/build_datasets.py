...
def main(feature_set): 
    ...
    if feature_set == 'none':
        ...
        X_train.to_csv('active_datasets/transactional_features_train.csv', header = False, index = False)
        X_test.to_csv('active_datasets/transactional_paper_features_test.csv', header = False, index = False)
    else:
        ...
        if feature_set == 'original':
            ...
            X_train.to_csv('active_datasets/original_paper_features_train.csv', header = False, index = False)
            X_test.to_csv('active_datasets/original_paper_features_test.csv', header = False, index = False)
        else:
            ...
            df.drop(columns =['customer', 'merchant', 'amount'], inplace=True)
            X_train, X_test = build_Xs('dataset_makers/original_data.csv', graph_features=df, cuts = [2, 3, 2])
            X_train.to_csv('active_datasets/all_features_train.csv', header = False, index = False)
            X_test.to_csv('active_datasets/all_features_test.csv', header = False, index = False)
