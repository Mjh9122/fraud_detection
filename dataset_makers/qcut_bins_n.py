...
def main(cuts):
    ...
    merchant_fraud_rate = pd.qcut(train.groupby('merchant').mean('fraud')['fraud'], cuts, labels=False, duplicates='drop')
    ...
    customer_previous_fraud = pd.qcut(train.groupby('customer').mean('fraud')['fraud'], cuts, labels=False, duplicates='drop')
    ...
    category_fraud_rate = pd.qcut(train.groupby('category').mean('fraud')['fraud'], cuts, labels=False, duplicates='drop')
    ...
    X_train_df.to_csv(f'./active_datasets/qcut_bins_{cuts}_train.csv', index = None, header=None)
    X_test_df = pd.DataFrame(X_test_transformed)
    X_test_df.to_csv(f'./active_datasets/qcut_bins_{cuts}_test.csv', index = None, header=None)
